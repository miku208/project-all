...
async function startConnection(options = {}) {
  ...
  // Auto-start web jadibot bareng bot utama. Idempotent - aman dipanggil
  // ulang tiap reconnect, server cuma bener2 listen sekali.
  import("./lib/ourin-jadibot-server.js")
    .then(({ startJadibotApiServer }) => startJadibotApiServer())
    .catch((e) =>
      colors.logger.error("jadibot-web", `Gagal start web jadibot: ${e.message}`),
    );
