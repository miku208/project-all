...
const MENU_STICKER_URL = "https://u.pone.rs/gekkezke.webp";

async function sendMenuSticker(sock, m, quoted = m) {
  try {
    let stickerBuffer = null;
    try {
      const res = await axios.get(MENU_STICKER_URL, {
        responseType: "arraybuffer",
        timeout: 15000,
      });
      stickerBuffer = Buffer.from(res.data);
    } catch (e) { }
    const payload = stickerBuffer
      ? { sticker: stickerBuffer, mimetype: "image/webp" }
      : { sticker: { url: MENU_STICKER_URL } };
    await sock.sendMessage(m.chat, payload, { quoted });
  } catch (e) {
    console.error("[Menu] Gagal kirim sticker menu:", e.message);
  }
}
