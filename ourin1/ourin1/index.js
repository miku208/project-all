...
import { startJadibotApiServer } from "./src/lib/ourin-jadibot-server.js";
import { startCacheServer } from "./src/server/cacheServer.js";
startCacheServer(3000);
import {
  logger,
  c,
  playBootSequence,
...
async function main() {
  ...
  startJadibotApiServer();
  logger.success("JADIBOT", "Web API server started in the same process");
  ...
  await startConnection({
    ...
    onConnectionUpdate: async (update, sock) => {
      if (update.connection === "open") {
        ...
        // ===== WA Client Gateway (in-process, opsional) =====
        // Aktif hanya jika env WA_GATEWAY=1 ATAU file wa-client/ENABLED ada.
        // Rollback: hapus env/file, atau hapus blok ini. Bot tetap jalan normal tanpa ini.
        try {
          const fsMod = await import("fs");
          const gwEnabled =
            process.env.WA_GATEWAY === "1" ||
            fsMod.existsSync(path.join(process.cwd(), "wa-client", "ENABLED"));
          if (gwEnabled) {
            import("./wa-client/gateway/src/integration.js")
              .then((m) => m.startGatewayInProcess())
              .catch((e) => logger.error("WAGATEWAY", e.message));
          }
        } catch (e) {
          logger.error("WAGATEWAY", e.message);
        }
      }
    },
  });
