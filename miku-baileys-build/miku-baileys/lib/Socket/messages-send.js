...
export const makeMessagesSocket = (config) => {
  ...
  const miku = new Dugong(waUploadToServer, relayMessage, config, sock);
  ...
  return {
    ...
    sendStatusMention: async (content, jids = []) => {
      return await miku.sendStatusWhatsApp(content, jids);
    },
    swgc: async (jid, content, options = {}) => {
      ...
      return await miku.handleGroupStory(storyContent, jid, null);
    },
    ...
    // Rich message senders (tables, code blocks, links, albums, interactive)
    // Rich message senders (tables, code blocks, links, albums, interactive)
    ...
    sendMessage: async (jid, content, options = {}) => {
      ...
      const messageType = miku.detectType(content);
      if (
        ...
        switch (messageType) {
          case "PAYMENT": {
            const paymentContent = await miku.handlePayment(content, quoted);
            return await relayMessage(jid, paymentContent, {
              messageId: generateMessageIDV2(userJid),
            });
          }
          case "PRODUCT": {
            const productContent = await miku.handleProduct(
              content,
              jid,
              quoted,
            );
          ...
          case "CAROUSEL": {
            const carouselContent = await miku.handleCarousel(
              content,
              jid,
              quoted,
            );
          ...
          case "INTERACTIVE": {
            const interactiveContent = await miku.handleInteractive(
              content,
              jid,
              quoted,
            );
          ...
          case "INTERACTIVE_BUTTONS": {
            const ibContent = await miku.handleInteractiveButtons(
              content,
              jid,
              quoted,
            );
          ...
          case "ALBUM":
            return await miku.handleAlbum(content, jid, quoted);
          case "EVENT":
            return await miku.handleEvent(content, jid, quoted);
          case "POLL_RESULT":
            return await miku.handlePollResult(content, jid, quoted);
          case "GROUP_STORY":
            return await miku.handleGroupStory(content, jid, quoted, options);
        }
      } else {
        let mediaHandle;
