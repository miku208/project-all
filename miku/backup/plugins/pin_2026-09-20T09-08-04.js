...
const pluginConfig = {
  ...
  category: "search",
  description: "Cari gambar di Pinterest dengan kualitas tinggi dan fitur lengkap",
  usage: ".pin <query>",
...
const DEFAULT_NEWSLETTER_ID = "120363400911374213@newsletter";
const DELAY_BETWEEN_IMAGES = 600;
const DELAY_BETWEEN_ALBUMS = 1000;
const MAX_RETRIES = 3;
...
async function downloadImageWithRetry(imageUrl, retries = MAX_RETRIES) {
...
async function searchPinterestWithCache(query) {
  const cacheKey = getCacheKey(query);

  if (searchCache.has(cacheKey)) {
    const cached = searchCache.get(cacheKey);
    if (isCacheValid(cached.timestamp)) {
      console.log(`[Pinterest] Menggunakan cache untuk: ${query}`);
...
async function sendAlbumMessageOptimized(m, sock, mediaList) {
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, DELAY_BETWEEN_ALBUMS));
    ...
    for (let i = 0; i < mediaList.length; i++) {
      try {
        ...
        console.error(`[Pinterest] Error mengirim gambar ${i + 1}:`, itemError.message);
        continue;
      }
    }
    ...
    console.error("[Pinterest] Album error:", error.message);
    return false;
  }
}

async function sendImagesOneByOneOptimized(m, sock, mediaList, config) {
  try {
    if (mediaList.length === 0) return false;

    const saluranId = config?.saluran?.id || DEFAULT_NEWSLETTER_ID;
    ...
    for (let i = 0; i < mediaList.length; i++) {
      try {
        ...
        await sock.sendMessage(
          ...
          {
            image: content.image,
            caption: `📌 Pinterest Search • ${i + 1}/${mediaList.length}`,
            contextInfo: {
    ...
    console.error("[Pinterest] Send one-by-one error:", error.message);
    return false;
  }
}
...
async function handler(m, { sock, config }) {
  ...
  try {
    const results = await searchPinterestWithCache(sanitized);
    ...
    for (const item of results) {
      const buffer = await downloadImageWithRetry(item.image_url);
      if (buffer) {
    ...
    let success = false;
    success = await sendAlbumMessageOptimized(m, sock, mediaList);

    if (!success) {
      console.log(`[Pinterest] Album gagal, mengirim ${mediaList.length} gambar satu per satu...`);
      success = await sendImagesOneByOneOptimized(m, sock, mediaList, config);
    }
