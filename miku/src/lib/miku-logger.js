...
import * as timeHelper from "./miku-time.js";
import { getCachedJid, isLidConverted } from "./miku-lid.js";
...
async function playBootSequence(info = {}) {
  const { name = "MIKU", version = "3.3", mode = "public" } = info;
  console.log("");
  console.log(chalk.cyan(`
         ...
         ╚██████╔╝╚██████╔╝██║  ██║██║██║ ╚████║
          ╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝╚═╝  ╚═══╝
`));
  console.log(`         ${chalk.magenta.bold("►")} ${chalk.white("MIKU MULTI-DEVICE BOT")} ${chalk.gray(`v${version}`)}`);
