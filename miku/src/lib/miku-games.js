import {
  ...
  getRandomReward,
  getProgressiveHint,
} from "./miku-game-data.js";
import { getDatabase } from "./miku-database.js";
import { addExpWithLevelCheck } from "./miku-level.js";
import {
  getGameContextInfo,
  sendGamePreview,
  checkFastAnswer,
} from "./miku-context.js";
import config from "../../config.js";
import fs from "fs";
import sharp from "sharp";
let fetchBuffer;
try {
  fetchBuffer = (await import("./miku-utils.js")).fetchBuffer;
} catch { }
        ...
        if (session && session.gameType === gameType) {
          ...
          await sock.sendPreview(
            m.chat,
            {
              ...
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["miku2"])).resize(300, 300).toBuffer(),
              previewType: 0,
            },
          );
          return;
...
const games = new MikuGames();

export { MikuGames, games };
