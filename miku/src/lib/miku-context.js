...
const keys = [
  ["miku-games", (buf) => { gameThumbBuffer = buf; }],
  ["miku-rpg", (buf) => { rpgThumbBuffer = buf; }],
  ["miku-winner", (buf) => { winnerThumbBuffer = buf; }],
];
...
function getRpgContextInfo(title, body) {
  ...
  if (title || body) {
    base.externalAdReply = {
      title: title || config.bot?.name || "Miku RPG",
      body: body || "",
...
function createFakeQuoted(botName = "Miku-AI", verified = true) {
  return {
    key: {
      fromMe: false,
