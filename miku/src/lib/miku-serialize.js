...
import { saluranCtx } from "./miku-context.js";
import { getAssetBuffer } from "./miku-asset-manager.js";
...
function createContextInfo(jid, text, title = "", body = "", thumbnail = null) {
  ...
  if (jid && text) {
    ...
    contextInfo.stanzaId = "MIKUAI" + Date.now();
  }
...
async function serialize(sock, msg, store = {}) {
  ...
  m.botName = sock.user?.name || config.bot?.name || "Miku-AI";
  m.messageId = m.id;
  m.chatId = m.chat;
  m.senderId = m.sender;
    ...
    if (replyVariant === 2) {
      ...
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
          ...
          jpegThumbnail: srtImage ? await sharp(srtImage).resize(300, 300).toBuffer() : await sharp(getAssetBuffer("miku2"))
            .resize(300, 300)
            .toBuffer(),
          caption: text,
          ...defaultOptions,
      ...
      const sss = getAssetBuffer("miku3");
      return sock.sendMessage(
        await ensureResolved(m.chat),
        {
          video: getAssetBuffer("miku-mp4"),
          caption: text,
          gifPlayback: true,
          contextInfo: {
            ...contextInfo,
      ...
      const thumbnailBuf = srtImage || getAssetBuffer("miku");
      const fakeOrder = {
        key: {
          participant: "0@s.whatsapp.net",
          remoteJid: m.chat,
        ...
        message: {
          orderMessage: {
            ...
            status: 1,
            surface: 1,
            message: config.bot?.name || "Miku-AI",
      ...
      const thumbnailBuf = randomImg || srtImage || await getAssetBuffer("miku");
      ...
      // ── REPLY V12 — A2UI Widget (bloksWidget) ──
      try {
        // Widget terlalu besar = WA menolak render. Teks > 4000 char -> fallback plain.
        if (text.length > 4000) throw new Error("text too long for A2UI widget");

        const { A2UI, sendA2UIWidget } = await import("./a2ui.js");
        const botName12 = config.bot?.name || "Miku-AI";

        const ui = new A2UI();
        const childIds = [
          ui.text(`💬 ${botName12}`, { variant: "h5" }),
          ui.divider(),
          ui.card(ui.text(text, { variant: "body" })),
        ];
        ui.root([ui.column(childIds)]);

        return await sendA2UIWidget(sock, await ensureResolved(m.chat), {
          a2ui: ui,
          bodyText: text.length > 120 ? text.slice(0, 117) + "..." : text,
          footer: `🧩 A2UI Widget • ${botName12}`,
          contextInfo: {
            mentionedJid: options?.mentions || [m?.sender] || [],
            isForwarded: true,
            forwardingScore: 9,
        ...
        // fallback ke reply teks biasa kalau widget gagal dibangun/dikirim
        return sock.sendMessage(
          await ensureResolved(m.chat),
          {
            text,
