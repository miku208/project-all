...
import { downloadMediaMessage, getContentType } from "mikuhostt-baileys";
import { addExifToWebp, DEFAULT_METADATA } from "./miku-exif.js";
...
/**
 * Deteksi error enkripsi/session Signal.
 * Session stale menyebabkan pesan penerima gagal didekripsi
 * (muncul "Menunggu pesan ini..." di WhatsApp penerima).
 */
function isSessionErrorErr(err) {
  const msg = String(err?.message || "") + String(err?.output?.payload?.error || "");
  return (
    err?.output?.statusCode === 500 ||
    /encrypt|session|pkmsg|No Signal|Bad MAC|No session|SessionError|No matching sessions|MessageCounterError|invalid key/i.test(
      msg,
    )
  );
}
...
/**
 * Hapus session Signal untuk 1 JID (PN + LID mapping-nya) supaya
 * session baru dibuat otomatis pada pengiriman berikutnya.
 */
async function clearSessionForJid(sock, jid) {
  if (!jid) return false;
  ...
  try {
    const { jidNormalizedUser } = await import("mikuhostt-baileys");
    const norm = jidNormalizedUser(jid);
    if (norm && norm !== jid) targets.add(norm);
  } catch { }
  ...
  for (const t of targets) {
    try {
      ...
      console.log("[SessionFix] clear failed:", t, e.message);
    }
  }

  // Paksa distribusi sender key baru di grup
  try {
...
/**
 * Re-fetch session dari server WA lalu kirim ulang.
 */
async function resendWithNewSession(sock, jid, content, options) {
  // 1. Bangun ulang session (fetch prekey bundle + inject)
  try {
    if (sock.assertSessions) await sock.assertSessions([jid], true);
  } catch (e) {
    console.log("[SessionFix] assertSessions failed:", e.message);
  }
  try {
...
async function extendSocket(sock) {
  ...
  sock.sendMessage = async (jid, content, options) => {
    try {
      ...
      const isSessionError = isSessionErrorErr(err);

      if (isSessionError) {
        // Hapus session stale lalu rebuild + kirim ulang sekali.
        try {
          await clearSessionForJid(sock, jid);
          return await resendWithNewSession(sock, jid, content, options);
        } catch (retryErr) {
          console.log(
            "[SessionFix] resend failed:",
            jid,
  ...
  sock.sendStickerPack = async (jid, stickers, m, options = {}) => {
    ...
    const publisher = options.publisher ?? options.author ?? "Miku-AI";
  ...
  sock.sendButton = async function (
    ...
    if (!options.footer) msg.footer = config.bot?.name || "Miku-AI";
    if (source) {
      let data = source;
