 ...
 *
 * Runtime:
 * - Baileys: mikuhostt-baileys (bawaan bot)
 *
 * A2UI widget builder adapted from FgsiDev's a2ui library.
...
/**
 * A2UI — builder untuk pesan widget A2UI (bloksWidget) WhatsApp.
 */
class A2UI {
  constructor({
  ...
  build({ uuid = crypto.randomUUID(), surfaceId, type = "im_a2ui", wrapped = true } = {}) {
    ...
    const data = wrapped
      ...
      : { components };
    return { uuid, data: JSON.stringify(data), type };
  }
}

/**
 * Kirim pesan widget A2UI melalui socket Baileys.
 */
async function sendA2UIWidget(client, jid, {
  ...
  const uploadMedia = async (client, media, mediaType = "image") => {
    let buffer;
    if (Buffer.isBuffer(media)) {
      ...
      throw new TypeError("Media harus berupa Buffer atau object { url }");
    }

    // prepareWAMessageMedia memanggil waUploadToServer dengan signature
    // yang benar: upload(filePath, { mediaType, fileEncSha256B64, timeoutMs })
    // dan mengembalikan proto.Message penuh — ambil mediaMessage-nya saja
    // (mis. imageMessage) agar cocok dengan header InteractiveMessage.
    const full = await prepareWAMessageMedia(
      { [mediaType]: buffer },
      {
    ...
    const mediaMessage = full[`${mediaType}Message`];
    if (!mediaMessage) {
      throw new Error(`Gagal menyiapkan media ${mediaType} untuk widget A2UI`);
    }
    return { [`${mediaType}Message`]: mediaMessage };
  };

  if (image) {
    imageMessage = await uploadMedia(client, image, "image");
  }

  if (video) {
    videoMessage = await uploadMedia(client, video, "video");
    if (gifPlayback) {
      videoMessage.videoMessage.gifPlayback = true;
    }
  }

  const params = {};

  const nativeFlowMessage = buttons?.length
    ? {
  ...
  if (typeof bottom_sheet === "object" && bottom_sheet?.name) {
    params.bottom_sheet = {
      in_thread_buttons_limit: 1,
      divider_indices: [1, 2],
      list_title: bottom_sheet.name,
      button_title: bottom_sheet.name,
    };
  }

  if (typeof limited_time_offer === "object" && limited_time_offer?.time) {
    params.limited_time_offer = {
      text: limited_time_offer.text || "",
      url: "",
      copy_code: "",
      expiration_time: Date.now() + limited_time_offer.time,
    };
  }

  nativeFlowMessage.messageParamsJson = JSON.stringify(params);

  const hasMedia = Boolean(imageMessage || videoMessage);
      ...
      additionalNodes: [
        {
          ...
          content: [
            ...
            },
            {
              tag: "quality_control",
              attrs: {
                decision_id: crypto.randomUUID().replace(/-/g, ""),
