...
import { generateWAMessageFromContent, prepareWAMessageMedia } from "mikuhostt-baileys";
import { serialize, getCachedThumb } from "./lib/miku-serialize.js";
import { saluranCtx } from "./lib/miku-context.js";
import {
  getPlugin,
  getPluginCount,
  getAllPlugins,
  pluginStore,
  getAllCommandNames,
} from "./lib/miku-plugins.js";
import {
  findSimilarCommands,
  formatSuggestionMessage,
} from "./lib/miku-similarity.js";
import { getDatabase } from "./lib/miku-database.js";
import {
  formatUptime,
  createWaitMessage,
  createErrorMessage,
} from "./lib/miku-formatter.js";
import { getUptime } from "./connection.js";
import { logger, logMessage, c } from "./lib/miku-logger.js";
import {
  isLid,
  isLidConverted,
  lidToJid,
...
import { hasActiveSession, getSession } from "./lib/miku-game-data.js";
import {
  levenshtein,
  formatAfkDuration,
  checkPermission,
  checkMode,
} from "./lib/miku-middleware.js";
import {
  handleAntilink,
  handleAntiJudol,
  handleAntiPhising,
  ...
  handleAntiHidetag,
  handleAntiSwGc,
} from "./lib/miku-group-protection.js";
import {
  debounceMessage,
  getCachedUser,
  getCachedGroup,
  getCachedSetting,
} from "./lib/miku-performance.js";
import { handleCommand as handleCaseCommand } from "../case/miku.js";
import { RateLimiterMemory } from "rate-limiter-flexible";
import { games as mikuGames } from "./lib/miku-games.js";
import fs from "fs";
import path from "path";
import { exec } from "child_process";
import axios from "axios";
import * as timeHelper from "./lib/miku-time.js";
const safe = (fn) => {
  try {
    return fn();
  } catch {
...
try {
  levelHelper = await import("./lib/miku-level.js");
} catch { }
try {
...
try {
  topupProofAnswerHandler = (await import("../plugins/user/topup.js"))
    .topupProofAnswerHandler;
} catch (e) { }
try {
...
try {
  handleAutoAI = (await import("./lib/miku-auto-ai.js")).handleAutoAI;
} catch { }
try {
  handleAutoDownload = (await import("./lib/miku-auto-download.js"))
    .handleAutoDownload;
} catch { }
try {
  checkStickerCommand = (await import("./lib/miku-sticker-command.js"))
    .checkStickerCommand;
} catch { }
try {
  handleStickerReply = (await import("./lib/miku-sticker-reply.js"))
    .handleStickerReply;
} catch { }
try {
...
async function handleSmartTriggers(m, sock, db) {
  ...
  try {
    ...
    const botName = config.bot?.name || "Miku-AI";
...
async function messageHandler(msg, sock) {
  try {
          ...
          if (buffer && buffer.length > 1000) {
            ...
            if (transcript) {
              ...
              if (bestMatch) {
                ...
                const { parseCommand } =
                  await import("./lib/miku-serialize.js");
    ...
    if (m.body?.startsWith(">>") && m.isOwner) {
      ...
      try {
        ...
        const execCode = new AsyncFunction(
          ...
          const { generateWAMessage, getBuffer, generateWAMessageFromContent, proto, generateMessageID } = await import("mikuhostt-baileys")
    ...
    try {
      if (topupProofAnswerHandler) {
        const handled = await topupProofAnswerHandler(m, sock);
        if (handled) return;
      }
    } catch (e) {
      console.error("[Handler] Topup proof error:", e.message);
    }
      ...
      if (checkStickerCommand) {
        try {
          ...
          if (stickerCmd) {
            ...
            const { parseCommand } = await import("./lib/miku-serialize.js");
            const parsed = parseCommand(m.body, prefix);
            m.isCommand = parsed.isCommand;
            m.command = parsed.command;
            m.args = parsed.args;
    ...
    for (let participant of participants) {
      ...
      const rankActions = {
        promote: {
          ...
          imgKey: "_promoteImg",
          imgPath: config.assets["miku-promote"],
          emoji: "🎉",
          label: "PROMOTE",
          text: (p, a) =>
        ...
        demote: {
          ...
          imgKey: "_demoteImg",
          imgPath: config.assets["miku-demote"],
          emoji: "📉",
          label: "DEMOTE",
          text: (p, a) =>
            ...
            viewOnceMessage: {
              message: {
                ...
                interactiveMessage: {
                  ...
                  footer: { text: config.bot?.name || "Miku-AI" },
