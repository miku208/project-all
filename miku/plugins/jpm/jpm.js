...
import { fetchGroupsSafe } from "../../src/lib/miku-jpm-helper.js";
import {
  getAutoJpmConfig,
  setAutoJpmConfig,
...
} from "../../src/lib/miku-auto-jpm.js";
import { getMimeType, getExtension } from "../../src/lib/miku-utils.js";
import * as timeHelper from "../../src/lib/miku-time.js";
import {
  getBinaryNodeChild,
...
import { saluranCtx } from "../../src/lib/miku-context.js";
import util from "util";
import axios from "axios";
import path from "path";
import fs from "fs";
import { getAssetBuffer } from "../../src/lib/miku-asset-manager.js";
...
try {
  cachedThumb = getAssetBuffer("miku2");
} catch {}
...
async function startAutoJpmSession(m, sock, db) {
  ...
  return sendInteractiveMessage(m, sock, {
    title: `🔄 ${config.bot?.name || "Miku-AI"} AutoJPM`,
    body,
    footer: `${config.bot?.name || "Miku-AI"} AutoJPM`,
    buttons,
  });
}
...
async function handleSetDelay(m, sock, db, input) {
  ...
  if (!input) {
    ...
    return sendInteractiveMessage(m, sock, {
      title: `⏱️ ${config.bot?.name || "Miku-AI"} Delay`,
      body,
      footer: `${config.bot?.name || "Miku-AI"} JPM System`,
      buttons,
    });
  }
