import { getCaseCount, getCasesByCategory } from "../../case/miku.js";
import {
...
import {
  formatUptime,
  getTimeGreeting,
} from "../../src/lib/miku-formatter.js";
import {
  getCommandsByCategory,
  getCategories,
} from "../../src/lib/miku-plugins.js";
...
async function formatTime(date) {
  const timeHelper = await import("../../src/lib/miku-time.js");
...
async function buildMenuText(
  ...
  txt += `    ᯓ INFO BOT\n`;
  txt += `╭  • Name : ${botConfig.bot?.name || "Miku-AI"}\n`;
  txt += `┆  • Author : ${botConfig.bot?.developer || "Owner"}\n`;
  txt += `┆  • Versi : ${botConfig.bot?.version || "1.2.0"}\n`;
  txt += `┆  • Type script : MIKU x ${botConfig.bot?.developer || "Owner"}\n`;
...
function getContextInfo(
  ...
  const saluranName =
    botConfig.saluran?.name || botConfig.bot?.name || "Miku-AI";
  ...
  const ctx = {
    ...
    isForwarded: true,
    externalAdReply: {
      title: botConfig.bot?.name || "Miku-AI",
...
async function handler(m, { sock, config: botConfig, db, uptime }) {
  ...
  try {
    imageBuffer = fs.readFileSync(botConfig.assets["miku"])
    thumbBuffer = fs.readFileSync(botConfig.assets["miku2"])
  } catch (e) {
      ...
      case 1:
        if (imageBuffer) {
          await sock.sendMessage(m.chat, {
            image: fs.readFileSync(config.assets["miku"]),
            caption: ``,
      ...
      case 3:
        const content = {
          buttonsMessage: {
            ...
            locationMessage: {
              jpegThumbnail: await sharp(fs.readFileSync(config.assets["miku"])).resize(300, 170).toBuffer(),
              name: config.bot.name,
...
> 🍬 *Register*: ${user.isRegistered ? "Sudah" : "Belum"}`,
      ...
      case 4: {
        const thumbnail = await sharp(fs.readFileSync(config.assets["miku"])).resize(300, 300).toBuffer()
        const qvideo = {
          key: {
            fromMe: false,
            participant: m.sender
        ...
        const media4 = await prepareWAMessageMedia({
          video: fs.readFileSync(config.assets["miku-mp4"]),
          gifPlayback: true
        }, { upload: sock.waUploadToServer });
...
`
        ...
        const { getAssetBuffer } = await import("../../src/lib/miku-asset-manager.js");
        const imageBuffer = await getAssetBuffer("miku2");
        const sharp = (await import("sharp")).default;
        const stickerBuf = await sharp(imageBuffer).resize(512, 512).webp().toBuffer();

        const { prepareWAMessageMedia } = await import("mikuhostt-baileys");
        ...
        const { generateWAMessageFromContent } = await import("mikuhostt-baileys");
        const menuMedia = await prepareWAMessageMedia({
          image: await getAssetBuffer("miku")
        }, { upload: sock.waUploadToServer });

        const videoLive = await prepareWAMessageMedia({
          video: await getAssetBuffer("miku-mp4")
        }, { upload: sock.waUploadToServer });
      ...
      case 9: {
        ...
        const { getAssetBuffer } = await import("../../src/lib/miku-asset-manager.js");
        const imageBuffer = await getAssetBuffer("miku");
        const favB = await getAssetBuffer("miku2");
        const sharp = (await import("sharp")).default;
        ...
        const { prepareWAMessageMedia, generateWAMessageFromContent } = await import("mikuhostt-baileys");
      ...
      case 8: {
        // ── MENU V8 — A2UI Widget (bloksWidget) ──
        const { A2UI, sendA2UIWidget } = await import("../../src/lib/a2ui.js");

        const runtimeStr8 = (seconds) => {
          seconds = Number(seconds);
          const d = Math.floor(seconds / (3600 * 24));
          if (d > 0) return `${d} hari`;
          const h = Math.floor((seconds % (3600 * 24)) / 3600);
          ...
          if (mn > 0) return `${mn} menit`;
          return `${Math.floor(seconds % 60)} detik`;
        };

        const toMonoBold8 = (text) => {
          const chars = {
            A: "𝗔", B: "𝗕", C: "𝗖", D: "𝗗", E: "𝗘", F: "𝗙", G: "𝗚", H: "𝗛", I: "𝗜", J: "𝗝", K: "𝗞", L: "𝗟", M: "𝗠",
            N: "𝗡", O: "𝗢", P: "𝗣", Q: "𝗤", R: "𝗥", S: "𝗦", T: "𝗧", U: "𝗨", V: "𝗩", W: "𝗪", X: "𝗫", Y: "𝗬", Z: "𝗭",
          };
        ...
        const botName8 = botConfig.bot?.name || "Miku-AI";
        const botDev8 = botConfig.bot?.developer || "Owner";
        const botVer8 = botConfig.bot?.version || "1.2.0";
        const prefix8 = m.prefix || botConfig.command?.prefix || ".";

        let roleInfo8 = { label: "User", emoji: "👤" };
 if (m.isOwner) roleInfo8 = { label: "Owner", emoji: "👑" };
        else if (m.isPremium) roleInfo8 = { label: "Premium", emoji: "💎" };

        const totalFitur8 = categories.sorted.reduce((acc, c) => acc + c.cmds.length, 0);

        const timeHelper8 = await import("../../src/lib/miku-time.js");
        const timeStr8 = timeHelper8.formatTime("HH:mm");
        ...
        let topCmds8 = [];
        try {
          const rawStats8 = fs.readFileSync(path.join(process.cwd(), "database/main/stats.json"), "utf8");
          const statsData8 = JSON.parse(rawStats8);
          topCmds8 = Object.entries(statsData8)
            .filter(([key]) => key.startsWith("command_"))
            .map(([key, count]) => ({ name: key.replace("command_", ""), count }))
            .sort((a, b) => b.count - a.count)
            .slice(0, 3);
        ...
        const tHeader = a2ui.text(`🌅 ${greeting}, *${m.pushName || "Kawan"}*!`, { variant: "h5" });
        const tIntro = a2ui.text(
          `Selamat datang di *${botName8}* 🌿 asisten WhatsApp buatan *${botDev8}*.\nAda *${totalFitur8} fitur* siap kubantu hari ini ✨`,
          { variant: "body" }
        );

        const botCard = a2ui.card(
          a2ui.text(
            `🤖 ${toMonoBold8("BOT INFO")}\n` +
            `▢ Nama    ─ ${botName8}\n` +
            `▢ Versi   ─ ${botVer8}\n` +
            `▢ Mesin   ─ mikuhostt-baileys\n` +
            `▢ Runtime ─ ${runtimeStr8(uptime)}`,
            { variant: "body" }
          )
        );

        const userCard = a2ui.card(
          a2ui.text(
            `👤 ${toMonoBold8("USER INFO")}\n` +
            `▢ Nama   ─ ${m.pushName || "User"}\n` +
            `▢ Status ─ ${roleInfo8.emoji} ${roleInfo8.label}\n` +
            `▢ Rank   ─ ${rankOf8(user.level || 0)} (Lv.${user.level || 0})\n` +
            `▢ Koin   ─ ${user.koin || 0} 🪙\n` +
            `▢ Energi ─ ${user.energi || 0} ⚡`,
            { variant: "body" }
          )
        ...
        const topText8 = topCmds8.length
          ? `\n🔥 ${toMonoBold8("PALING SERING")}\n` +
            topCmds8.map((c, i) => `${medals8[i] || "•"} ${prefix8}${c.name} · ${c.count}x`).join("\n")
          : "";

        const menuCard = a2ui.card(
          a2ui.text(
            `📂 ${toMonoBold8("DAFTAR MENU")}\n` +
            categories.sorted
              .map(({ cat, cmds, emoji }, i) => `${String(i + 1).padStart(2, "0")}. ${emoji} ${cat} · ${cmds.length} cmd`)
              .join("\n") +
            topText8,
            { variant: "body" }
          )
        ...
        const tTips = a2ui.text(
          `💡 Tips: ${prefix8}menucat <kategori> untuk lihat isi kategori.\nContoh: ${prefix8}menucat tools`,
          { variant: "body" }
        );
        const tTime = a2ui.text(`⏰ ${timeStr8} WIB • ${runtimeStr8(uptime)}`, { variant: "body" });
        ...
        try {
          await sendA2UIWidget(sock, m.chat, {
            a2ui,
            bodyText: `🧩 ${greeting}, ${m.pushName || "Kawan"}! Menu lengkap ${botName8} — ${totalFitur8} fitur dalam ${categories.sorted.length} kategori.`,
            footer: `🧩 A2UI Widget • ${botName8} • ${prefix8}menu`,
            ...
            buttons: [
              {
                ...
                params: {
                  display_text: "🧀 Contact Owner",
                  url: `https://wa.me/${botConfig.owner?.number?.[0]}`,
                  merchant_url: `https://wa.me/${botConfig.owner?.number?.[0]}`,
                },
              },
              {
                name: "quick_reply",
                params: {
                  display_text: "🍽️ Allmenu",
                  id: `${prefix8}allmenu`,
                },
              },
            ],
          ...
          console.error("[Menu] V8 A2UI error:", e.message);
          await m.reply(text);
        }
        break;
      }
    ...
    if (audioEnabled) {
      const audioUrl = botConfig.assets["miku-mp3"];
      try {
        switch (menuVariant) {
          case 1:
            try {
          ...
          case 2: {
            ...
            try {
              ...
              await sock.sendMessage(m.chat, {
                audio: fs.readFileSync(config.assets["miku-mp3"]),
                mimetype: "audio/mpeg",
                ptt: false,
              }, { quoted: qpoll });
            }
