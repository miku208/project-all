...
import { getCasesByCategory } from "../../case/miku.js";
import { prepareWAMessageMedia, generateWAMessageFromContent } from "mikuhostt-baileys";
...
import { getTimeGreeting } from "../../src/lib/miku-formatter.js";
import fs from "fs"
...
async function handler(m, { sock, db }) {
      ...
      switch (menucatVariant) {
        ...
        case 2: {
          const media = await prepareWAMessageMedia(
            {
              image: fs.readFileSync(config.assets["miku2"]),
            },
        ...
        case 5: {
          ...
          const media4 = await prepareWAMessageMedia({ video: fs.readFileSync(config.assets["miku-mp4"]), gifPlayback: true }, { upload: sock.waUploadToServer });
                ...
                interactiveMessage: {
                  ...
                  contextInfo: {
                    ...
                    forwardedNewsletterMessageInfo: {
                      ...
                      newsletterName: config.saluran?.name || config.bot?.name || "Miku-AI",
                      serverMessageId: 127,
                    },
                  },
                  nativeFlowMessage: {
        ...
        case 7: {
          // ── MENUCAT V7 — A2UI Widget (daftar kategori) ──
          const { A2UI, sendA2UIWidget } = await import("../../src/lib/a2ui.js");
          const ui7 = new A2UI();

          // Widget terlalu besar = WA menolak render. Total cmd dibatasi:
          // kalau lebih dari 150, tiap kategori cukup tampil ringkas (count saja).
          const totalCmds7 = visibleCats.reduce(
            (acc, cat) => acc + (commandsByCategory[cat] || []).length + (casesByCategory[cat] || []).length,
            0
          );
          const fullList7 = totalCmds7 <= 150;

          const catCards7 = visibleCats.map((cat, i) => {
            const pluginCmds = commandsByCategory[cat] || [];
            const caseCmds = casesByCategory[cat] || [];
            const allCmds = [...pluginCmds, ...caseCmds];
            const emoji = CATEGORY_EMOJIS[cat] || "📋";
            const children = [
              ui7.text(`${String(i + 1).padStart(2, "0")}. ${emoji} ${cat.toUpperCase()} · ${allCmds.length} cmd`, { variant: "h4" }),
            ];
            if (fullList7) {
              children.push(
                ui7.text(allCmds.map((cmd) => `⇨ ${prefix}${cmd}${getCommandSymbols(cmd)}`).join("\n"), { variant: "body" })
              );
            }
            return ui7.card(ui7.column(children));
          });

          ui7.root([
            ui7.column([
              ui7.text("📂 DAFTAR KATEGORI", { variant: "h5" }),
              ui7.text(`Total ${visibleCats.length} kategori. Ketik ${prefix}menucat <kategori> untuk buka isinya.`, { variant: "body" }),
              ui7.divider(),
              ...catCards7,
              ui7.divider(),
              ui7.text(`🧩 A2UI Widget · ${config.bot?.name || "Bot"}`, { variant: "caption" }),
            ]),
          ]);

          await sendA2UIWidget(sock, m.chat, {
            a2ui: ui7,
            bodyText: `📂 Daftar Kategori — ${visibleCats.length} kategori tersedia.`,
            footer: `🧩 A2UI Widget • ${config.bot?.name || "Bot"}`,
            buttons: [
              { name: "quick_reply", params: { display_text: "🍅 Kembali Ke Menu Utama", id: m.prefix + "menu" } },
            ],
            quoted: m,
          });
          break;
  ...
  try {
    switch (menucatVariant) {
      ...
      case 7: {
        // ── MENUCAT V7 — A2UI Widget (isi kategori) ──
        const { A2UI, sendA2UIWidget } = await import("../../src/lib/a2ui.js");
        const ui7 = new A2UI();

        const cmdId7 = ui7.text(commandLines.join("\n"), { variant: "body" });
        const children7 = [
          ui7.text(`${emoji} ${matchedCat.toUpperCase()}`, { variant: "h5" }),
          ui7.text(`Total ${allCommands.length} commands`, { variant: "body" }),
          ui7.divider(),
          cmdId7,
        ];
        if (caseCommands.length > 0) {
          children7.push(ui7.text(`(${pluginCommands.length} plugin + ${caseCommands.length} case)`, { variant: "caption" }));
        }
        ui7.root([ui7.column(children7)]);

        await sendA2UIWidget(sock, m.chat, {
          a2ui: ui7,
          bodyText: `${emoji} Kategori ${matchedCat} — ${allCommands.length} commands.`,
          footer: `🧩 A2UI Widget • ${config.bot?.name || "Bot"}`,
          buttons: [
            { name: "quick_reply", params: { display_text: "📂 Kembali Ke Daftar Kategori", id: m.prefix + "menucat" } },
            { name: "quick_reply", params: { display_text: "🍅 Kembali Ke Menu Utama", id: m.prefix + "menu" } },
          ],
          quoted: m,
        });
