...
const fontUrl = getAssetBuffer("miku-font");
...
async function handler(m, { sock }) {
  ...
  const inputUrl = getAssetBuffer("miku-kertas");
  if (!inputUrl) {
    return m.reply(
  ...
  try {
    ...
    if (!_fontRegistered) {
      try {
        const fontBuf = getAssetBuffer("miku-font");
        if (fontBuf) {
    ...
    const bgBuf = getAssetBuffer("miku-kertas");
